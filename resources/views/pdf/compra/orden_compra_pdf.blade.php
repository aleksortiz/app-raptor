...
<body style="margin: 10px;">
        ...
        <tr>
          ...
          <td style="padding: 0px; vertical-align: top; text-align: center;">
            <div style="padding: 0px; width: 520px;">
              <p class="titulo-cabezal">ININTEC S. de R.L de C.V</p>
              <p class="texto-cabezal">Calle Ricardo Leon No. 6214<br>
              Infonavit Casas Grandes, Ciudad Juárez Chih. Tel: (656) 618-87-88<br>
              e-mail: inintec.admon1@gmail.com</p>
            </div>
          </td>
        </tr>
      </table>
  ...
  <div>
    @if ($po->canceled_at)        
      <center>
        <h3 style="color: rgb(101, 94, 94);">*** ORDEN CANCELADA ***</h3>
      </center>
    @elseif (!$po->authorized_by)        
      <center>
        <h3 style="color: red;">*** ORDEN NO AUTORIZADA ***</h3>
      </center>
    @endif
    
    <h3>Orden de Compra: {{$po->nombre}}</h3>
    <p class="texto"><b>PROVEEDOR:</b> {{strtoupper($po->proveedor->nombre)}}</p>
    @if ($po->contactos->count() > 0)        
    <p class="texto"><b>CONTACTO{{ $po->contactos->count() > 1 ? "S" : ""}}:</b>
      @foreach ($po->contactos as $item)
          {{$item->prefijo_nombre_correo}}<br>
      @endforeach
    </p>
    @endif
    <p class="texto"><b>FOLIO:</b> {{$po->nombre}}</p>
    <p class="texto"><b>FECHA:</b> {{$po->fecha_format}}</p>
    {{-- <p class="texto"><b>MONEDA:</b> {{$po->moneda}}</p> --}}
  </div>
...
<div>
  ...
  <table width="100%" style="border: 1px solid; padding: 0px; margin: 0px; border-spacing: 0px;">
    ...
    <tbody style="background-color: #fff; border: none;">
      ...
      @foreach ($po->conceptos as $concepto)
      <tr style="font-size: 11px;">
        <td width="30px" align="center">{{$loop->iteration}}</td>
        <td width="380px" style="padding: 6px;" align="left">{{$concepto->descripcion}}</td>
        <td width="60px" align="center">{{$concepto->unidad_venta}}</td>
        <td width="60px" align="center">{{$concepto->cantidad}}</td>
        <td align="center">$</td>
        <td width="65px" align="right">{{number_format($concepto->precio, 2)}}</td>
        <td align="center">$</td>
        <td width="65px" align="right">{{number_format($concepto->subtotal, 2)}}</td>
      </tr>
      @endforeach
    </tbody>
  </table>
  ...
  <table width="100%" style="margin-0px; padding: 0px; border-spacing: 0px;">
    <tr style="vertical-align: top;">
      <td style="font-size: 10px;">
        @if ($po->notas)
          <h2>Notas:</h2>
          <p style="white-space: pre-wrap; line-height: 2;">{{$po->notas}}</p>
        @endif
      </td>
      <td style="padding: 0px; margin: 0px;" align="right;">
        <table style="margin-left:auto; border-spacing: 0px; font-size: 11px; border-bottom: solid 1px; border-right: solid 1px; border-left: solid 1px;">
          <tr>
            ...
            <td width="65px" align="right;">@float($po->subtotal)</td>
          </tr>
          <tr>
            <td width="80px" align="right;" style="padding: 3px; background-color: {{$color}}; color: white;"><b>IVA (@float($po->tasa_iva)%): </b></td>
            <td width="25px" align="center;">$</td>
            <td width="65px" align="right;">@float($po->iva)</td>
          </tr>
          <tr>
            <td width="80px" align="right;" style="padding: 3px; background-color: {{$color}}; color: white;"><b>TOTAL: </b></td>
            <td width="25px" align="center;">$</td>
            <td width="65px" align="right;">@float($po->total)</td>
          </tr>
        </table>
      </td>
    </tr>
