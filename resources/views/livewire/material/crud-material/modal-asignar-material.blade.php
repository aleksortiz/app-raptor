                ...
                <div>
                    <div class="row">
                        <div class="form-group col-2">
                            <label>Folio</label>
                            <input wire:model.defer="folioSearch" wire:keydown.enter="searchFolioHandler" style="text-transform: uppercase; text-align: center;" type="text" class="form-control" />
                        ...
                        <tbody>
                            @foreach ($this->folios as $row)
                                <tr>
                                    ...
                                    <td>{{ $row->vehiculo }}</td>
                                    <td>
                                        <input style="text-align: center;" wire:keydown.enter="asignarMaterial({{$loop->index}})" wire:model.defer="folios.{{$loop->index}}.cantidad" type="text" class="form-control form-control-sm" onkeypress="return event.charCode >= 46 && event.charCode <= 57" />
                                    </td>
                                    <td>
                                        <a target="_blank" href="servicios/{{$row->id}}?activeTab=6" class="btn btn-xs btn-primary"><i class="fa fa-car"></i> Ver Entrada</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
