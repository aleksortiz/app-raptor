...
@section('title', __("Catalogo de Citas"))
<div class="pt-3" x-data="{ citas: @entangle('citas') }">
        ...
        <script>
            ...
            function addEvent(cita){
                calendar.addEvent({
                    ...
                    color: cita.tipo == 'VALUACION' ? 'blue' : 'yellow'
                });
            }
            ...
            Livewire.on('updateCalendarEvent', function(event) {
                ...
                const tipoValuacion = event[0].tipo == 'VALUACION';
                const fecha = event[0].fecha_valuacion;

                if (eventCalendar) {
                    eventCalendar.setProp('title', vehiculo);
                    eventCalendar.setProp('color', valuacionEfectuada ? 'blue' : 'green');
